...
#### 7.1 ローカル開発環境でのテスト
- [x] `viewer/`ディレクトリで`npm install`実行
- [x] `.env.local`を設定
- [x] `npm run dev`で開発サーバー起動（ポート3300）
- [x] `old_viewer`と並行して動作確認
- [x] 各機能のテスト:
  - [x] トップページ（コレクション一覧）の表示
  - [x] コレクション詳細ページの表示
  - [x] ページネーションの動作
  - [x] 検索機能の動作
  - [x] UIデザインの一致確認（`old_viewer`と比較）
...
#### 7.4 エラーハンドリングの検証
- [x] ChromaDB接続エラー時の挙動（ChromaDBを停止して確認）
- [ ] LiteLLM接続エラー時の挙動
- [ ] 存在しないコレクションへのアクセス
...
#### 8.2 CLAUDE.mdの更新
- [x] プロジェクト概要をNext.js用に更新
- [x] アーキテクチャセクションの更新:
  - Express → Next.js App Router
  - EJS → React Server Components
  - axios → Fetch API
- [x] コアコンポーネントの説明更新
- [x] ルート構造の説明（App Router）
- [x] 開発コマンドの更新
- [x] Docker設定の説明更新
- [x] プロジェクト構造の更新

#### 8.3 .env.exampleの更新
- [x] Next.js用の環境変数テンプレートに更新（フェーズ1で完了）
- [x] 既存の環境変数を維持
- [x] コメントを追加
